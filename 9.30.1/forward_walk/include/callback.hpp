...
private: //외부에서 사용 안하는 함수들
    ...
    // rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr initial_direction_subsciption_;

    std::atomic<bool> line_turn{false};
    std::atomic<int> turns_remaining_{0};

public: // 외부에서 써야하는 함수들
    ...
    // void InitialDirectionCallback(const sensor_msgs::msg::Imu::SharedPtr Imu_msg);

    void CalculateTurn(int a);

    int GetIndexT() const { return indext; }
    int GetAngle() const { return angle; } // CalculateTurn에서 계산한 각도를 뺴오기 위한 getter 함수
