...
#include "robot_msgs/msg/motion_during.hpp"  //motion end 불러오기
#include "robot_msgs/msg/ball_result.hpp" //공을 잡았는지 안 잡았는지 구분을 위한
...
public:
    ...
    : Node("main_node"), motion_in_progress_(false)
    {
        ...
        // motion_during 퍼블리셔 생성
        motion_during_pub_ = this->create_publisher<robot_msgs::msg::MotionDuring>("/motion_during", 10);
...
private:
    ...
    rclcpp::Publisher<robot_msgs::msg::MotionDuring>::SharedPtr motion_during_pub_;
    ...
    void MotionLoop()
    {
        ...
        if (callback_->IsMotionFinish())
        {
            ...
            robot_msgs::msg::MotionDuring during_msg;

            // pick 모션 시퀀스
            if (pick_mode)
            {
                if (current_go_ == 9 && ball_catch_ == true){
            ...
            during_msg.motion_during = false;
            motion_during_pub_->publish(during_msg);
            RCLCPP_INFO(this->get_logger(),
                    "motion_end publish = %d", during_msg.motion_during);
