    ...
    : Node("main_node"), motion_in_progress_(false)
    {
        ...
        motion_start_timer_ = this->create_wall_timer(
            ...
            [this]() {
                ...
                RCLCPP_INFO(this->get_logger(), "motion end 1회 퍼블리쉬");
...
private:
    ...
    rclcpp::Publisher<robot_msgs::msg::MotionCommand>::SharedPtr motion_command_pub;
    ...
    void MotionLoop()
    {
        ...
        if (callback_->IsMotionFinish())
        {
            ...
            if (pick_mode)
            {
                ...
                // 만약 공을 잡았는데 못ㄱ 던졌을 시 res를 받아서 니가 알아서해 강제 카운트 니가 알아서해
                if (current_go_ == 5){
                    if(count1 == 0){
                        sleep(1);
                        current_go_ = 2;
                        callback_->SetLineTurn(true);
                        callback_->OnLineResult(53);
                        callback_->ResetMotion();
                        RCLCPP_INFO(this->get_logger(), "current_go_ =%d", current_go_);
