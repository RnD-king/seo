...
Dims YoloLayerPlugin::getOutputDimensions(int index, const Dims* inputs, int nbInputDims) TRT_NOEXCEPT {
    ...
    int totalsize = mMaxOutObject * sizeof(RawDetection) / sizeof(float);
...
__global__ void CalDetection(const float* input, float* output, int noElements, const int netwidth, const int netheight,
                             int maxoutobject, int yoloWidth, int yoloHeight, const float anchors[kNumAnchor * 2],
                             int classes, int outputElem){ //, bool is_segmentation) {
    ...
    for (int k = 0; k < kNumAnchor; ++k) {
        ...
        if (count >= maxoutobject)
            return;
        char* data = (char*)res_count + sizeof(float) + count * sizeof(RawDetection);
        RawDetection* det = (RawDetection*)(data);
        ...
        det->confidence = box_prob * max_cls_prob;
...
void YoloLayerPlugin::forwardGpu(const float* const* inputs, float* output, cudaStream_t stream, int batchSize) {
    int outputElem = 1 + mMaxOutObject * sizeof(RawDetection) / sizeof(float);
    ...
    for (unsigned int i = 0; i < mYoloKernel.size(); ++i) {
        ...
        CalDetection<<<(numElem + mThreadCount - 1) / mThreadCount, mThreadCount, 0, stream>>>(
                inputs[i], output, numElem, mYoloV5NetWidth, mYoloV5NetHeight, mMaxOutObject, yolo.width, yolo.height,
                (float*)mAnchor[i], mClassCount, outputElem); //, is_segmentation_);
    }
}
...
IPluginV2IOExt* YoloPluginCreator::createPlugin(const char* name, const PluginFieldCollection* fc) TRT_NOEXCEPT {
    ...
    YoloLayerPlugin* obj =
            new YoloLayerPlugin(class_count, input_w, input_h, max_output_object_count,kernels); // is_segmentation, kernels);
