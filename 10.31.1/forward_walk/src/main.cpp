...
private:
    ...
    void MotionCallback(const robot_msgs::msg::MotionCommand::SharedPtr msg)        
    {
        ...
        switch (msg->command) 
        {
            ...
            case 34: command_ = 34; break; //Back_Right
        ...
        if(last_mode == false){
            ...
            {
                RCLCPP_INFO(this->get_logger(), "last_mode xxxx 회전");
                angle_ = msg->angle;
                angle__ = angle_;
        ...
        else if(last_mode == true){
            if (current_go_ == 2 || current_go_ == 29)
            {
                RCLCPP_INFO(this->get_logger(), "last_mode 좌회전");
                current_go_ = 2;
                last_angle = msg->line_angle;
                callback_->SetLineTurn(true); // line_turn 키기
                callback_->OnLineResult(last_angle);
                callback_->ResetMotion();
            }
            else if(current_go_ == 3 || current_go_ == 30)
            {
                RCLCPP_INFO(this->get_logger(), "last_mode 우회전");
                current_go_ = 3;
                last_angle = msg->line_angle;
                callback_->SetLineTurn(true); // line_turn 키기
                callback_->OnLineResult(last_angle);
            ...
            {
                RCLCPP_INFO(this->get_logger(), "last_mode 나머지");
                current_go_ = 25;
                callback_->ResetMotion();
            }
        }
