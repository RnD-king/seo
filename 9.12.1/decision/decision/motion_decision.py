...
from robot_msgs.msg import MotionCommand, LineResult, FallResult, BallResult, MotionDuring
...
class MotionDecision(Node):
    def __init__(self):
        ...
        # motion_during subscibe
        self.motion_during_sub = self.create_subscription(MotionDuring, '/motion_during', self.MotionDuringCallback, 10)

    # pick motion이 끝나면 pick_end를 받아오기
    def MotionDuringCallback(self, during_msg: MotionDuring):
        self.motion_during = during_msg.motion_during
        # self.motion_during = during_msg.motion_during
        self.get_logger().info(f"motion_during: {during_msg.motion_during}")

    def BallCallback(self, Ball_msg: BallResult):
        # if(self.motion_during):
        if(self.motion_during == False):
