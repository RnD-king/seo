...
static std::string getAvailablePort() {
    std::vector<std::string> candidates = {"/dev/ttyUSB0", "/dev/ttyUSB1", "/dev/ttyUSB2"};

    for (const auto& port : candidates) {
        auto portHandler = dynamixel::PortHandler::getPortHandler(port.c_str());
        if (portHandler->openPort()) {
            std::cout << "[Info] Connected at " << port << std::endl;
            return port; // 성공한 포트 반환
        }
    }

    std::cerr << "[Error] No valid Dynamixel port found (tried USB0 and USB1)" << std::endl;
    exit(1);
}
...
Dxl::Dxl()
{
    ...
    // ✅ 자동 포트 선택
    std::string device_name = getAvailablePort();
