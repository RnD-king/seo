...
from robot_msgs.msg import LinePoint, LinePointsArray, BallResult, MotionDuring # type: ignore
...
class LineListenerNode(Node): #################################################################### 판단 프레임 수 바꿀 때 yolo_cpp 도 고려해라~~~
    def __init__(self):
        ...
        self.armed = False               # motion_during 방어~!
        self.window_id = 0
        self.frame_idx = 0       
        ...
        self.motion_during_sub = self.create_subscription(  # 모션 끝나면 받아올 T/F
            MotionDuring,                   
            '/motion_during',                   
    ...
    def motion_callback(self, msg: MotionDuring): # 모션 끝 같이 받아오기 (중복 방지)
        if not bool(msg.motion_during):
            self.armed = True
            self.get_logger().info("Subscribed /motion_during !!!!!!!!!!!!!!!!!!!!!!!!")
